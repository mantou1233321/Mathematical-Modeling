...
def build_nonlinear_mixed_model(data):
   # 步骤1: 提取因变量和自变量
    ...
    # 定义嵌套模型（假设去掉某些变量）
    reduced_X = X[:, :8]  # 仅保留前8列变量（线性主效应）
    reduced_p = reduced_X.shape[1]
    
    # 计算嵌套模型的参数估计
    try:
        reduced_XtX = reduced_X.T @ reduced_X
        reduced_Xty = reduced_X.T @ y
        reduced_XtX_inv = np.linalg.inv(reduced_XtX)
        reduced_beta = reduced_XtX_inv @ reduced_Xty
        reduced_y_pred = reduced_X @ reduced_beta
        reduced_residuals = y - reduced_y_pred
        reduced_RSS = np.sum(reduced_residuals ** 2)
    except np.linalg.LinAlgError:
        print("嵌套模型矩阵奇异，使用伪逆")
        reduced_XtX_inv = np.linalg.pinv(reduced_XtX)
        reduced_beta = reduced_XtX_inv @ reduced_Xty
        reduced_y_pred = reduced_X @ reduced_beta
        reduced_residuals = y - reduced_y_pred
        reduced_RSS = np.sum(reduced_residuals ** 2)
    ...
    # 计算似然比统计量
    LRT_stat = (reduced_RSS - RSS) / MSE
    df_diff = p - reduced_p  # 自由度差
    p_value_LRT = 1 - stats.chi2.cdf(LRT_stat, df_diff)
    
    print(f"似然比统计量 = {LRT_stat:.2f}")
